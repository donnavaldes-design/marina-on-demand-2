create or replace function bmod_internal.membership_token_transition(p_user_id uuid, p_action text, p_lease uuid default null, p_data jsonb default '{}'::jsonb)
returns boolean language plpgsql security definer set search_path = '' as $$
declare c public.user_connections%rowtype;
begin
  if coalesce(auth.role(), '') <> 'service_role' then raise exception 'service role required'; end if;
  select * into c from public.user_connections where user_id=p_user_id and integration_key='bmod_membership' for update;
  ...
  if p_action='disconnect' then
    ...
    delete from public.connection_oauth_states where user_id=p_user_id and integration_key='bmod_membership';
    return true;
  end if;
  ...
  if p_action='save' then
    ...
    perform public.store_connection_secret(p_user_id,'bmod_membership',p_data->>'access_token');
    perform public.store_connection_refresh_secret(p_user_id,'bmod_membership',p_data->>'refresh_token');
    if nullif(p_data->>'client_secret','') is not null then
      perform public.store_connection_client_secret(p_user_id,'bmod_membership',p_data->>'client_secret');
    end if;
...
revoke all on function bmod_internal.membership_token_transition(uuid,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function bmod_internal.membership_token_transition(uuid,text,uuid,jsonb) to service_role;


create or replace function public.membership_token_transition(p_user_id uuid,p_action text,p_lease uuid default null,p_data jsonb default '{}'::jsonb)
returns boolean language sql security invoker set search_path = '' as $$
 select bmod_internal.membership_token_transition(p_user_id,p_action,p_lease,p_data);
$$;
revoke all on function public.membership_token_transition(uuid,text,uuid,jsonb) from public,anon,authenticated;
grant execute on function public.membership_token_transition(uuid,text,uuid,jsonb) to service_role;
insert into public.integration_catalog(integration_key,display_name,provider,customer_visible,read_only_default)
values ('bmod_membership','Corporate membership verification','highlevel',false,true)
on conflict (integration_key) do update set customer_visible=false;
create policy corporate_membership_service_only on public.user_connections as restrictive for all to authenticated
using (integration_key <> 'bmod_membership') with check (integration_key <> 'bmod_membership');
